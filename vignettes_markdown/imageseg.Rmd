...
output:
  ...
  html_document:
    ...
    df_print: kable
vignette: |
  %\VignetteIndexEntry{A sample session in imageseg} %\VignetteEngine{knitr::rmarkdown} \usepackage[utf8]{inputenc}
...
2) understory vegetation density from photographs of an orange-red flysheet at a distance from the observer (training data used 10m distance).

Part 1 of this vignette demonstrates the workflow and shows how the pre-trained models can be used for creating predictions based on images. 

Part 2 of this vignette shows how more advanced users can easily create new model architectures and conduct their own deep learning experiments. 

Part 3 of this vignette shows how users can use the pre-trained models we provide as a foundation and continue training with their own data.

Part 3 of this vignette demonstrates how models with multiple classes can be trained and used for predictions (using a toy data set).
...
It can help to add miniconda to environment paths. Below is a function that temporarily adds directories to the PATH variable . Replace "YourUserName" with the correct user name. 
...
Try updating the R package Rcpp:
...
Download the pre-trained models in order to use them on your own canopy images. We provide two separate models, one for canopy cover and one for understory vegetation density. Below are the download links to the hdf5 file containing the trained models (both the model architecture and learned weights).
...
**Canopy model**: https://www.dropbox.com/s/q4xztzzbmxzxwdy/imageseg_canopy_model.hdf5?dl=1

**Understory model**: https://www.dropbox.com/s/wb38mlb6agl09y0/imageseg_understory_model.hdf5?dl=1


Download the model you need and put the file in a suitable location. We will later use a function to load it into R. File size is about 90 Mb per model.



## Downloading the training data

This is only needed if you wish to train your own models. In that case you can download the training data from:

**Canopy training data**  https://www.dropbox.com/s/j96ncev8dxlr44y/canopy_training_data.zip?dl=0

**Understory training data**  https://www.dropbox.com/s/n2s5uqb4ber8nb1/understory_training_data.zip?dl=0


If you only want to use the models for predictions on your own photos, you don't need to download the training data. 


## Running imageseg on a graphics card (GPU)

While it is possible to run everything on CPU, lots of deep learning applications benefit from running on graphics cards (GPUs). More specifically, Nvidia GPUs are supported via the CUDA library. On our machines, we used:

* R 4.1.1
* CUDA v10.1.105 
* Keras 2.4.3 (installed automatically by install_keras())
* TensorFlow 2.2.0 (installed automatically by install_keras())
* R package keras (2.4.0)
...
# Part 1: Sample workflow for canopy cover classification
...
The understory vegetation model differentiates vegetation and an orange flysheet (and probably more generally, orange objects).
...
So far, all predictions were binary. Internally, it is a model with 1 class, so in the examples above U-Net learned to recognize sky or the orange flysheet. In models with multiple classes, we can for example train the model to recognize vegetation and the orange flysheet as two separate classes. Applications are not limited to forestry, it can be used for arbitrary image segmentation problems, e.g. in medical images or microscopy as demonstrated below.

Here we will give two examples demonstrating the fundamentals of image segmentation with multiple classes. The first uses microscopic images in which separate cells, bacteria and background. The second example uses MNIST database of handwritten digits which is included in Keras. It is normally used for image classification ("Which digit is written in the image?"), but for demonstration we will use it in image segmentation. 
...
Callbacks can be useful to prevent overfitting. Here we reduce the learning rate when training stagnates (callback_reduce_lr_on_plateau), and stop training it training stagnates a bit longer (callback_early_stopping). The maximum number of epochs is so low (for demonstration) that we will likely not need early stopping though, but it can help in some circumstances.
...
The call to model fit is a little different than before due to the sampling generator (which is passed to argument x, with y being empty). We also define the validation data explicitly and use callbacks.
...
Again, the left column of the following two example images shows the model input, the central column shows the model prediction (as probability), the right column shows the binary classification (only areas with probability > 0.9 are indicated).
...
Model performance is not perfect, but decent for demonstration. It is a relatively small data set, we did not do any data augmentation, which would almost certainly have improved predictions, and did not use any callbacks to e.g. reduce learning rate and further improve accuracy.
